...
class PebbleBundle(object):
	...
	def add_firmware(self,firmware_path,firmware_type,firmware_timestamp,firmware_commit,firmware_hwrev):
		...
		self.bundle_manifest['firmware']={'name':os.path.basename(firmware_path),'type':firmware_type,'timestamp':firmware_timestamp,'commit':firmware_commit,'hwrev':firmware_hwrev,'size':flen(firmware_path),'crc':stm32crc(firmware_path),}
	...
	def add_resources(self,resources_path,resources_timestamp,sdk_version=None):
		...
		if sdk_version is not None and sdk_version['major']>=5 and sdk_version['minor']>19:
			timeline_resource_path=os.path.join('build',self.subfolder,'timeline_resources.json')
			if os.path.exists(timeline_resource_path):
				with open(timeline_resource_path,'r')as f:
					timeline_resources=json.load(f)
				self.bundle_manifest['app_resources']=timeline_resources
...
def check_required_args(opts,*args):
	...
	for required_arg in args:
		try:
			...
			raise Exception("Missing argument {}".format(required_arg))
def make_firmware_bundle(firmware,firmware_timestamp,firmware_commit,firmware_type,board,resources=None,resources_timestamp=None,outfile=None,verbose=False):
	bundle=PebbleBundle()
	firmware_path=os.path.expanduser(firmware)
	bundle.add_firmware(firmware_path,firmware_type,firmware_timestamp,firmware_commit,board)
	if resources:
